...
def evaluation(model, data_loader):
    model.eval()
    result = []
    for i, (inputs, question_ids, instructions_list) in tqdm(
        enumerate(data_loader), disable=not accelerator.is_local_main_process
    ):
        generated_ids = model.generate(
            **inputs, max_new_tokens=30, instructions_list=instructions_list
        )
        ...
        for answer, ques_id in zip(answers, question_ids):
            if type(data_loader.dataset) == COCO:
                result.append(
                    {
                        "image_id": int(ques_id),
                        "caption": answer.replace("a picture of ", ""),
                    }
                )
...
def main(args, config):
    ...
    result_file = save_result(eval_result, args.result_dir, "result")
    if type(test_loader.dataset) == TextVQA:
        accelerator.wait_for_everyone()
        evaluator = TextVQAEvaluator(
            dataset_json_file="data/textvqa/train/TextVQA_0.5.1_val.json"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", default="./configs/textvqa/pretrain_clip_flan_t5_base.yaml"
    )
    ...
    run_name = (
        ...
        + config_name
        + "_"
        + datetime.datetime.now().strftime("%m_%d_%Y_%H")
    )
